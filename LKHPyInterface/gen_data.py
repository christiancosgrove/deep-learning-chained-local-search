...
def make_geometric_data(points, edges, best_nodes):
	...
	return Data(x=torch.tensor(features), edge_index=torch.tensor(edges, dtype=torch.long), y=torch.tensor(y, dtype=torch.float))
...
def make_data_delaunay(points, tour, best_nodes):
	...
	tri = Delaunay(points)
	# TODO: Fix double counting of edges shared by adjacent triangles
	# also fix double counting with tour
	for triangle in tri.simplices:
		edges.append((triangle[0], triangle[1]))
		edges.append((triangle[0], triangle[2]))
		edges.append((triangle[1], triangle[2]))

	return make_geometric_data(points, edges, best_nodes)
