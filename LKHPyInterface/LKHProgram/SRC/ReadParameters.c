...
void ReadParameters(char* parameterFileLines, int numParameterLines)
{
    ...
    ParameterFile = fmemopen(parameterFileLines, strlen(parameterFileLines), "r"); //outputs a stream like a file

    while ((Line = ReadLine(ParameterFile))) {
    // printff("Num lines %d \n", numParameterLines);
    // printff("First line %s \n", parameterFileLines[0]);
    // for (int lineLoc = 0; lineLoc < numParameterLines; lineLoc++) {
        // Line = parameterFileLines[lineLoc];
        printff("At line %s \n", Line);
