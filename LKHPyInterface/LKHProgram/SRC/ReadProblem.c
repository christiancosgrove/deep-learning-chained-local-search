...
void ReadProblem(char *problemFileLines, int numProblemLines)
{
    int i, K;
    char *Line, *Keyword;
    ...
    // FILE *stream;
    ProblemFile = fmemopen(problemFileLines, strlen(problemFileLines), "r");

    while ((Line = ReadLine(ProblemFile))) {
    // for (int lineLoc = 0; lineLoc < numProblemLines; lineLoc++) {
        // Line = problemFileLines[lineLoc];
        printff("At line: %s", Line);
